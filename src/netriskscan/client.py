...
class NetRiskScan:
    """Synchronous client for the NetRiskScan Developer API.

    Works without an API key (anonymous tier, metered by the server) or with
    one passed explicitly or via the ``NETRISKSCAN_API_KEY`` environment
    variable. See the README for the full precedence rules and anonymous-tier
    behavior.
    """
    ...
    def __enter__(self) -> NetRiskScan:
        return self
    ...
    def ip_risk(self, ip: str, *, timeout: float | None = None) -> IpRiskResult:
        ...
        data, meta = self._request("GET", url, timeout=timeout)
        result = IpRiskResult._from_json(data)
        _attach_meta(result, meta)
        return result

    def usage(self, *, timeout: float | None = None) -> UsageResult:
    ...
    def _request(self, method: str, url: str, *, timeout: float | None) -> tuple[Any, ResponseMeta]:
